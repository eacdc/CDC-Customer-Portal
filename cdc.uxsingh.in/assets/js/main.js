...
// --- Auth session helpers (logout handling) ---
(function () {
  ...
  function resolveRedirect(redirectUrl) {
    const defaultPath = 'auth-login-cover.html';
    let target = (redirectUrl || '').trim();

    if (!target) target = defaultPath;

    const isAbsolute = /^https?:\/\//i.test(target);
    
    // If not an absolute URL and starts with auth-login-cover, handle the path
    if (!isAbsolute && target.startsWith('auth-login-cover')) {
      // Check if we're already in the vertical-menu-template directory
      const currentPath = window.location.pathname;
      const isInVerticalMenuTemplate = currentPath.includes('/vertical-menu-template/');
      
      if (isInVerticalMenuTemplate) {
        // Already in vertical-menu-template, use relative path (just the filename)
        target = target;
      } else {
        // Not in vertical-menu-template, use absolute path from root
        target = `/html/vertical-menu-template/${target}`;
      }
    }

    if (!isAbsolute && !/\.html?(\?|#|$)/i.test(target)) {
  ...
  async function performLogout(redirectUrl) {
    ...
    const apiBase = (session && session.apiBase) || getDefaultApiBase();
    const endpoint = `${apiBase.replace(/\/$/, '')}/auth/logout`;

    if (session && session.token) {
      const body = session.sessionId ? { sessionId: session.sessionId } : {};
      try {
        ...
        // swallow network errors; still proceed with logout
      }
    }

    clearStoredSession();
    window.location.href = resolveRedirect(redirectUrl);
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    logoutLinks.forEach(link => {
      link.addEventListener('click', event => {
        event.preventDefault();
        const redirect = link.dataset.redirect || 'auth-login-cover.html';
